...
<style type="text/css">
	...
	<div class="mini-filter-popup">
		...
		overflow: scroll;">
		<span class="close-filter" onclick="closePopFilter()">&times;</span>
...
<script type="text/javascript">
	...
	function sort_info(){
		...
		let pageLang = document.documentElement.lang || (typeof pll_current_language === 'function' ? pll_current_language() : 'th');
		if (allpro == chk_display) {
			document.getElementById("tit-con").innerHTML = pageLang === 'th' ? "โครงการแนะนำจากแอสเซทไวส์ " : "Recommended Projects from AssetWise"
		}
		else{
			document.getElementById("tit-con").innerHTML = pageLang === 'th' ? "ผลการค้นหา " : "Search Results "	
		}
		document.getElementById("num-con").innerHTML = "- "+chk_display+" "+pageLang === 'th' ? "รายการ" : "Projects";
		if (chk_display == 0) {
	...
	<div class="cont-pd">
		...
		<div class="grid grid-rows-2 md:grid-rows-1 md:grid-cols-2 flex items-center">
			...
			<div class="row-span-1 md:col-span-1 flex justify-end">
				<div  id="sortby-box-wrap" class="box-fil-date pointer relative justify-between" data-sortby="-1" data-sorttype="date" onclick="turnon_sorting()" style="min-width: 215px;">
